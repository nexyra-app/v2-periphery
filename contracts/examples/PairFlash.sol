...
import '@nexyra/v2-core/contracts/interfaces/callback/INexyraV2FlashCallback.sol';
import '@nexyra/v2-core/contracts/libraries/LowGasSafeMath.sol';
...
/// @notice An example contract using the Nexyra V2 flash function
contract PairFlash is INexyraV2FlashCallback, PeripheryPayments {
    ...
    function nexyraV2FlashCallback(
        uint256 fee0,
        uint256 fee1,
        bytes calldata data
    ) external override {
    ...
    /// @notice Calls the pools flash function with data needed in `nexyraV2FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        INexyraV2Pool pool = INexyraV2Pool(PoolAddress.computeAddress(factory, poolKey));
