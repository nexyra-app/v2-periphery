...
import '@nexyra/v2-core/contracts/libraries/SafeCast.sol';
import '@nexyra/v2-core/contracts/libraries/TickMath.sol';
import '@nexyra/v2-core/contracts/interfaces/INexyraV2Pool.sol';
import '@nexyra/v2-core/contracts/interfaces/callback/INexyraV2SwapCallback.sol';
...
contract QuoterV2 is IQuoterV2, INexyraV2SwapCallback, PeripheryImmutableState {
    using Path for bytes;
    using SafeCast for uint256;
    using PoolTicksCounter for INexyraV2Pool;
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) private view returns (INexyraV2Pool) {
        return INexyraV2Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }

    /// @inheritdoc INexyraV2SwapCallback
    function nexyraV2SwapCallback(
        int256 amount0Delta,
        int256 amount1Delta,
        bytes memory path
        ...
        INexyraV2Pool pool = getPool(tokenIn, tokenOut, fee);
        ...
        )
    {
        ...
        INexyraV2Pool pool = getPool(params.tokenIn, params.tokenOut, params.fee);
